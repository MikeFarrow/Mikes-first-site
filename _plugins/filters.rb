  ...
  module Filters
    ...
    def get_page(page_path)
      ...
      if (!page_path.start_with?('/'))
        raise ArgumentError, "get_page must receive an absolute path beginning with '/', received '#{page_path}'"
      end
    ...
    def get_pages(folder_path)
      ...
      if (!folder_path.start_with?('/'))
        raise ArgumentError, "get_pages filter must receive an absolute path beginning with '/', received '#{folder_path}'"
      end
      ...
      if site.collections.key?(relative_folder_parts[0])
        ...
        return site.pages.select { |page| page.dir.sub(/^\//, '').sub(/\/$/, '') == relative_folder_path }
      end
